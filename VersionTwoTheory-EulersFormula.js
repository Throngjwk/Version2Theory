...
var init = () => {
    ...
    // a4
    {
        let getDesc = (level) => "a_4=10^{" + level + "}";
        let getInfo = (level) => "a_4=" + getQ2(level).toString(0);
        a4 = theory.createUpgrade(10, currency_I, new ExponentialCost(4000, 2.2));
        a4.getDescription = (_) => Utils.getMath(getDesc(a4.level));
        a4.getInfo = (amount) => Utils.getMathTo(getInfo(a4.level), getInfo(a4.level + amount));
    }
    
    // a5
    {
        let getDesc = (level) => "a_5=10^{" + level + "}";
        let getInfo = (level) => "a_5=" + getQ2(level).toString(0);
        a5 = theory.createUpgrade(11, currency_I, new ExponentialCost(4000, 2.2));
        a5.getDescription = (_) => Utils.getMath(getDesc(a5.level));
        a5.getInfo = (amount) => Utils.getMathTo(getInfo(a5.level), getInfo(a5.level + amount));
    }
...
var getA4 = (level) => BigNumber.TEN.pow(level);
var getA5 = (level) => BigNumber.TEN.pow(level);
